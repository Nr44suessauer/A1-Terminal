...
class ColorWheel(tk.Frame):
    ...
    def draw_color_wheel(self):
        ...
        # Farbkreis in Segmenten zeichnen
        for angle in range(0, 360, 2):  # 2-Grad-Schritte für Glätte
            # Äußerer Ring (gesättigte Farben)
            for r in range(self.radius - 20, self.radius):
                x = self.center + r * math.cos(math.radians(angle))
                y = self.center + r * math.sin(math.radians(angle))
                
                # HSV zu RGB umwandeln
                hue = angle
                saturation = (r - (self.radius - 20)) / 20  # 0-1
                value = 1.0  # Volle Helligkeit
            ...
            # Innere Bereiche (weniger gesättigt)
            for r in range(20, self.radius - 20, 5):
                x = self.center + r * math.cos(math.radians(angle))
                y = self.center + r * math.sin(math.radians(angle))
                
                # Saturation basierend auf Abstand vom Zentrum
                saturation = r / (self.radius - 20)
                value = 1.0 - (0.3 * (1 - saturation))  # Helligkeit variiert leicht
        ...
        # Weißer Kreis in der Mitte für bessere Sichtbarkeit
        center_radius = 20
